...
mpls_encap_rules_bareudp_ip_test () {
ovs-ofctl -O OpenFlow13 add-flow br_mpls1 "in_port=$ingress_port,dl_type=0x0800,nw_dst=1.1.1.2 actions=output:$bareudp_egress_port"
ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "in_port=$bareudp_egress_port,dl_type=0x0800 actions=goto_table:1"
#ovs-ofctl -O OpenFlow14 add-flow br_mpls2 "in_port=$bareudp_egress_port,dl_type=0x8847 actions=output:$ingress_port"
#ovs-ofctl  -O Openflow14 add-flow br_mpls2 "packet_type=(1,0x8847) actions=decap(packet_type(ns=0,type=0)),output:$ingress_port"
ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "in_port=$bareudp_egress_port,dl_type=0x0800 actions=goto_table:1"
...
ovs-vsctl add-port  br_mpls1 udp_port1 -- set interface udp_port1 type=bareudp options:remote_ip=2.1.1.3 options:local_ip=2.1.1.2 options:payload_type=0x0800 options:dst_port=6638  ofport_request=$bareudp_egress_port
ovs-vsctl add-port  br_mpls2 udp_port2 -- set interface udp_port2 type=bareudp options:remote_ip=2.1.1.2 options:local_ip=2.1.1.3 options:payload_type=0x0800  options:dst_port=6638  ofport_request=$bareudp_egress_port
