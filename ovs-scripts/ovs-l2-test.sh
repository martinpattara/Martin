...
N1_MAC=36:b1:ee:7c:01:03
N2_MAC=36:b1:ee:7c:01:02
...
create_mpls_tunnel() {
    ...
    local label=$3
    local gw=$4
    ip -netns mpls_vm$id route add $net encap mpls $label via inet $gw dev \
              Nokia-eth1
}
mpls_mpls_encap_rules() {
create_mpls_tunnel 1 1.1.1.2/32 200 1.1.1.5
ovs-ofctl -O OpenFlow13 add-flow br_mpls1 "in_port=$ingress_port,dl_type=0x8847,actions=decap, encap(mpls(ether_type=0x8847)),set_field:15->mpls_label,encap(ethernet),mod_dl_dst:$N2_MAC,mod_dl_src:$N1_MAC,output:$egress_port"
}

mpls_mpls_encap_rules_1() {
create_mpls_tunnel 1 1.1.1.2/32 200 1.1.1.5
ovs-ofctl -O OpenFlow13 add-flow br_mpls1 "in_port=$ingress_port,dl_type=0x8847,actions=encap(mpls(ether_type=0x8847)),set_field:15->mpls_label,encap(ethernet),mod_dl_dst:$N2_MAC,mod_dl_src:$N1_MAC,output:$egress_port"
}



mpls_encap_pop_rules () {
ovs-ofctl -O OpenFlow13 add-flow br_mpls1 "in_port=$ingress_port,dl_type=0x0800,actions=decap, encap(mpls(ether_type=0x8847)),encap(ethernet),mod_dl_dst:$N2_MAC,mod_dl_src:$N1_MAC,output:$egress_port"
ovs-ofctl -O OpenFlow13 add-flow br_mpls1 "in_port=$egress_port,dl_type=0x8847 actions=pop_mpls:0x0800,goto_table:1"
ovs-ofctl -O OpenFlow13 add-flow br_mpls1 "in_port=$ingress_port,dl_type=0x0806 actions=goto_table:1"
ovs-ofctl -O OpenFlow13 add-flow br_mpls1 "in_port=$egress_port,dl_type=0x0806 actions=goto_table:1"
ovs-ofctl -O OpenFlow13 add-flow br_mpls1 "table=1,actions=normal"

ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "in_port=$ingress_port,dl_type=0x0800,actions=decap, encap(mpls(ether_type=0x8847)),encap(ethernet),mod_dl_dst:$N1_MAC,mod_dl_src:$N2_MAC,output:$egress_port"
ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "in_port=$egress_port,dl_type=0x8847 actions=pop_mpls:0x0800,goto_table:1"
ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "in_port=$ingress_port,dl_type=0x0806 actions=goto_table:1"
ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "in_port=$egress_port,dl_type=0x0806 actions=goto_table:1"
ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "table=1,actions=normal"
}

mpls_push_decap_rules () {
ovs-ofctl -O OpenFlow13 add-flow br_mpls1 "in_port=$ingress_port,dl_type=0x0800,actions=push_mpls:0x8847,set_field:15->mpls_label,output:$egress_port"
ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "in_port=$ingress_port,dl_type=0x0800,actions=push_mpls:0x8847,set_field:15->mpls_label,output:$egress_port"
ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "in_port=$egress_port,dl_type=0x8847 actions=decap(),decap(packet_type(ns=1,type=0x0800)),encap(ethernet),goto_table:1"
ovs-ofctl -O OpenFlow13 add-flow br_mpls1 "in_port=$egress_port,dl_type=0x8847 actions=decap(),decap(packet_type(ns=1,type=0x0800)),encap(ethernet),output:$ingress_port"
ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "table=1,in_port=$egress_port, actions=output:$ingress_port"
}
mpls_encap_rules_arp () {
ovs-ofctl -O OpenFlow13 add-flow br_mpls1 "in_port=$ingress_port,actions=encap(mpls(ether_type=0x8847)),encap(ethernet),set_field:00:00:00:00:00:02->dl_dst,set_field:00:00:00:00:00:01->dl_src,output:$egress_port"
ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "in_port=$egress_port,dl_type=0x8847 actions=decap(),decap(packet_type(ns=0,type=0)),output:$ingress_port"
ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "in_port=$ingress_port,actions=encap(mpls(ether_type=0x8847)),encap(ethernet),set_field:00:00:00:00:00:02->dl_dst,set_field:00:00:00:00:00:01->dl_src,output:$egress_port"
ovs-ofctl -O OpenFlow13 add-flow br_mpls1 "in_port=$egress_port,dl_type=0x8847 actions=decap(),decap(packet_type(ns=0,type=0)),output:$ingress_port"
}
...
#ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "table=1,in_port=$egress_port actions=set_field:00:00:00:00:00:02->dl_dst,set_field:00:00:00:00:00:01->dl_src output:$ingress_port"
ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "table=1,in_port=$egress_port actions=normal"
}

multiple_mpls_encap_rules () {
#ovs-ofctl -O OpenFlow13 add-flow br_mpls1 "in_port=$ingress_port,dl_type=0x0800,nw_dst=1.1.1.2 actions=encap(mpls),set_field:13->mpls_label,encap(ethernet),push_mpls:0x8847,set_field:15->mpls_label,output:$egress_port"
#ovs-ofctl -O OpenFlow13 add-flow br_mpls1 "in_port=$egress_port actions=encap(mpls),output:$ingress_port"
#ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "in_port=$ingress_port actions=encap(mpls),output:$egress_port"
#ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "in_port=$egress_port actions=encap(mpls),output:$ingress_port"
...
#ovs-ofctl -O OpenFlow13 add-flow br_mpls1 "in_port=$ingress_port,dl_type=0x0800,nw_dst=1.1.1.2 actions=encap(mpls(ether_type=0x8847)),set_field:13->mpls_label,encap(ethernet),push_mpls:0x8847,set_field:15->mpls_label,output:$egress_port"
#ovs-ofctl -O OpenFlow13 add-flow br_mpls1 "in_port=$ingress_port,dl_type=0x0800,nw_dst=1.1.1.2 actions=encap(nsh(md_type=1)),output:$egress_port"
ovs-ofctl -O OpenFlow13 add-flow br_mpls1 "in_port=$ingress_port,actions=encap(mpls(ether_type=0x8847)),encap(mpls(ether_type=0x8847)),encap(ethernet),set_field:00:00:00:00:00:02->dl_dst,set_field:00:00:00:00:00:01->dl_src,output:$egress_port"
ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "in_port=$egress_port,dl_type=0x8847 actions=decap(),decap(packet_type(ns=1,type=0x8847)),decap(packet_type(ns=0,type=0)),goto_table:2"
ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "in_port=$egress_port,dl_type=0x0800 actions=goto_table:1"
ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "in_port=$egress_port,dl_type=0x0806 actions=goto_table:1"
#ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "table=1,in_port=$egress_port,dl_type=0x0800,nw_dst=1.1.1.2 actions=output:$ingress_port"
#ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "table=1,in_port=$egress_port actions=set_field:00:00:00:00:00:02->dl_dst,set_field:00:00:00:00:00:01->dl_src output:$ingress_port"
ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "table=1,in_port=$egress_port, dl_type=0x8847 actions=decap(packet_type(ns=0,type=0)),goto_table:2" 
ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "table=2,in_port=$egress_port actions=normal"
}
...
ovs-ofctl -O OpenFlow13 add-flow br_mpls1 "in_port=$ingress_port,dl_type=0x0800,nw_dst=1.1.1.2 actions=push_mpls:0x8847,set_field:13->mpls_label,output:udp_port1"
ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "in_port=$bareudp_egress_port,dl_type=0x8847 actions=pop_mpls:0x0800,goto_table:1"
#ovs-ofctl -O OpenFlow14 add-flow br_mpls2 "in_port=$bareudp_egress_port,dl_type=0x8847 actions=output:$ingress_port"
#ovs-ofctl  -O Openflow14 add-flow br_mpls2 "packet_type=(1,0x8847) actions=decap(packet_type(ns=0,type=0)),output:$ingress_port"
ovs-ofctl -O OpenFlow13 add-flow br_mpls2 "in_port=$bareudp_egress_port,dl_type=0x0800 actions=goto_table:1"
...
ovs-vsctl add-port  br_mpls1 udp_port1 -- set interface udp_port1 type=bareudp options:remote_ip=2.1.1.3 options:local_ip=2.1.1.2 options:payload_type=0x8847 options:dst_port=6635  ofport_request=$bareudp_egress_port
ovs-vsctl add-port  br_mpls2 udp_port2 -- set interface udp_port2 type=bareudp options:remote_ip=2.1.1.2 options:local_ip=2.1.1.3 options:payload_type=0x8847  options:dst_port=6635  ofport_request=$bareudp_egress_port
...
#mpls_push_decap_rules
mpls_encap_rules_arp
ip netns exec mpls_vm1 ping 1.1.1.2 -c 4
